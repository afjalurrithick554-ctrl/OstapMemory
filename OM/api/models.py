...
# Notebook models
class ImplementationPlanFile(BaseModel):
    path: str
    content: str
...
class ImplementationPlan(BaseModel):
    text: str
    files: Optional[list[ImplementationPlanFile]] = None
    # Финализация плана кнопкой «План готов» — гейт перехода 1→2.
    finalized: Optional[bool] = False


class ReportEntry(BaseModel):
    """Одна запись append-журнала отчёта: кто, когда и что зафиксировал."""
...
class ReportEntryInput(BaseModel):
    """Новая запись отчёта для дописывания через PUT. `at` проставляет бэкенд."""
...
class Checklist(BaseModel):
    """Именованный чек-лист. У ячейки/задачи всегда есть дефолтный (isDefault)."""
...
class NotebookCreate(BaseModel):
    ...
    notebook_type: Optional[str] = Field("cell", description="Type: cell, task, or subtask")
    parent_id: Optional[str] = Field(None, description="Parent notebook ID for subtasks")
    space_id: Optional[str] = Field(None, description="Project space ID (None = global)")
    is_admin: Optional[bool] = Field(
        False, description="Administrative cell (rendered in the opposite theme)"
    )
    checklist: Optional[list[Checklist]] = Field(
        None, description="Named checklists for the notebook"
    )
    goal: Optional[str] = Field(None, description="Goal text (Цель ячейки/задачи)")
    goal_file: Optional[Dict[str, Any]] = Field(
        None, description="Attached goal file metadata {name, path, size}"
    )
...
class NotebookUpdate(BaseModel):
    ...
    notebook_type: Optional[str] = Field(None, description="Type: cell, task, or subtask")
    parent_id: Optional[str] = Field(None, description="Parent notebook ID for subtasks")
    space_id: Optional[str] = Field(
        None, description="Project space ID; empty string moves card to global «Ячейки»"
    )
    is_admin: Optional[bool] = Field(
        None, description="Administrative cell (rendered in the opposite theme)"
    )
    implementation_plan: Optional[ImplementationPlan] = Field(
        None, description="Implementation plan for task notebooks"
    )
    deadline: Optional[str] = Field(None, description="Deadline date string")
    assigned: Optional[str] = Field(None, description="Assigned user ID")
    checklist: Optional[list[Checklist]] = Field(
        None, description="Named checklists for the notebook"
    )
    report_entry: Optional[ReportEntryInput] = Field(
        None,
        description="New report entry to append (append-only journal; server stamps `at`)",
    )
    review_notes: Optional[Dict[str, Any]] = Field(
        None, description="Review remarks entered when returning task 3->2 {text, url, file, created}"
    )
    done: Optional[bool] = Field(
        None, description="Toggle 'Готово' (drives transition 2->3)"
    )
    accepted: Optional[bool] = Field(
        None, description="Toggle 'Проверено' (drives transition 3->4)"
    )
    goal: Optional[str] = Field(None, description="Goal text (Цель ячейки/задачи)")
    goal_file: Optional[Dict[str, Any]] = Field(
        None, description="Attached goal file metadata {name, path, size}"
    )
    sort_order: Optional[int] = Field(None, description="Manual sort order within state group")
...
class NotebookResponse(BaseModel):
    ...
    goal_file: Optional[Dict[str, Any]] = None
    sort_order: Optional[int] = 0
    report: Optional[list[ReportEntry]] = Field(default_factory=list)
    ...
    @classmethod
    def _coerce_report(cls, v):
        # Устойчивость к легаси: до миграции 30 report был строкой. Если в БД
        # осталось не-list значение (пустая/непустая строка), не роняем валидацию
        # всей карточки (иначе один битый report обваливает весь список в 500) —
        # приводим к пустому журналу.
...
class NoteCreate(BaseModel):
    ...
    # Привязка к проекту (space kind=project). None → наследовать от пространства ноутбука.
    space_id: Optional[str] = Field(
        None, description="Project space ID (None = inherit from notebook's space)"
    )
...
class NoteUpdate(BaseModel):
    ...
    space_id: Optional[str] = Field(
        None, description="Project space ID; empty string = default (global)"
    )
...
class NoteResponse(BaseModel):
    ...
    space_id: Optional[str] = None
    # Непосредственный родитель заметки — ноутбук (ячейка/подзадача), к которому
    # она привязана (ребро artifact). Ярлык в UI показывает родителя, а не проект.
    parent_id: Optional[str] = None
