...
def parse_source_form_data(
    ...
    space_id: Optional[str] = Form(None),  # проект-пространство источника
...
async def get_sources(
    ...
    try:
        ...
        if notebook_id:
            ...
            query = f"""
                SELECT id, asset, created, title, updated, topics, command, space_id,
...
async def create_source(
    ...
    try:
        ...
        # Заодно наследуем проект (space_id) от первого ноутбука, у которого он задан —
        # чтобы источник, созданный внутри проекта, попадал в тот же проект.
        derived_space_id: Optional[str] = None
        for notebook_id in source_data.notebooks or []:
            ...
            if derived_space_id is None and getattr(notebook, "space_id", None):
                derived_space_id = notebook.space_id
        effective_space_id = source_data.space_id or derived_space_id
        ...
        if source_data.async_processing:
            ...
            try:
                ...
                return SourceResponse(
                    ...
                    space_id=getattr(processed_source, "space_id", None),
                    asset=AssetModel(
...
async def update_source(source_id: str, source_update: SourceUpdate):
    ...
    try:
        ...
        if source_update.space_id is not None:
            # Пустая строка = дефолтный (глобальный) источник (space_id = None).
            source.space_id = source_update.space_id or None
