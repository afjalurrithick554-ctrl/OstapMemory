...
async def stream_notebook_chat_response(
    ...
    Зеркалит паттерн чата источника (api/routers/source_chat.py): граф выполняется
    блокирующе, ответ ИИ отдаётся одним событием ai_message. SSE-канал даёт единый
    транспорт и задел под потокенную выдачу в будущем.
    """
    try:
        current_state = await asyncio.to_thread(
            chat_graph.get_state,
        ...
        # Событие пользовательского сообщения
        yield f"data: {json.dumps({'type': 'user_message', 'content': message, 'timestamp': None})}\n\n"

        # Выполняем граф в треде (SqliteSaver синхронный)
...
async def stream_chat_message(
    ...
    """Send a message to a notebook chat session with SSE streaming response."""
    try:
        full_session_id = (
            session_id
            if session_id.startswith("chat_session:")
